  ...
  def perform(post_params, media_attachments_params)
    ActiveRecord::Base.transaction do
      ...
      media_attachments_params.each do |blob_params|
        ...
        if media_attachment.file.content_type == "image/gif"
          filename = media_attachment.file.blob.filename.to_s
          Tempfile.open([filename, ".webm"], binmode: true) do |tempfile|
            ...
            filename = "#{media_attachment.id}.webm"
            blob = ActiveStorage::Blob.create_and_upload!(
              key: "blog/#{filename}",
              io: tempfile.to_io,
              filename: filename,
              metadata: {custom: {original_content_type: "image/gif"}}
            )
